<?php include("header.php"); ?>
	...
	<section id="allindex">
		...
		if (!isset($_GET['nom'])) {
				...
				$rep = $bdd->query('SELECT mouvement as nom, url_peintre as url FROM peintures GROUP BY mouvement');
		?>
		<?php while ($donnees = $rep->fetch()) { ?>
			<aside id="team2">

				<a href="<?php echo 'mouvements.php?nom=' . $donnees['nom'] ?> "><img id="imgindex" src="<?php echo utf8_encode($donnees['url']) ?>" /></a>
				<p id="paraindex" >
			...
			<?php }
					...
					$rep = $bdd->prepare('SELECT id, url, nom, nom_peintre FROM peintures WHERE mouvement = ?');
